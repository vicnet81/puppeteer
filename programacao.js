...
let main = async () => {
    let students = (await GoogleApi.cells("1_dIQ2dDT4GVXJMnda2bg8l28WNeAavCzabuT9mPxZLo","diário!B2:M"))
      .map(row=>{return {
        name: row[0],
        email: row[1],
        firstGrade: row[6],
    ...
    let courses = (await GoogleApi.cells("1_dIQ2dDT4GVXJMnda2bg8l28WNeAavCzabuT9mPxZLo","courses!B2:C"))
    .map(row=>{return {
      id: row[0],
      name: row[1]
    }});
